<template>
  <div class="card m-4">
    ...
    <div class="card-text">
      <p v-if="isCanceled">{{ $t('yourSubscriptionWasCanceled') }}</p>

      <div v-if="isActive">
        <label for="subscription-current-period" class="settings-label">{{ $t('currentPeriod') }}</label>
        <input variant="solo-filled" name="subscription-current-period" type="text"
          :placeholder="subscriptionCurrentPeriod" />
      </div>

      <div v-if="hasEnded">
        <label for="subscription-ended-at" class="settings-label">{{ $t('endedAt') }}</label>
        <input variant="solo-filled" name="subscription-ended-at" type="text" :placeholder="subscriptionEndedAt" />
      </div>

      <div v-if="isCanceled">
        <label for="subscription-ended-at" class="settings-label">{{ $t('willEndAt') }}</label>
        <input variant="solo-filled" name="subscription-ended-at" type="text"
          :placeholder="subscriptionCurrentPeriodEnd" />
      </div>

      <div v-if="hasLastInvoice" class="m-4 flex flex-col text-left">
        ...
        <label class="settings-label">
          {{ $t('invoiceUrlColon') }} <a :href="hostedInvoiceUrl" target="_blank" class="border-0">{{ $t('clickHere')
          }}</a>
        </label>
    ...
    <div class="card-actions">
      <button class="settings-button rounded shadow-md" v-if="subscription && !isCanceled" id="cancelSubscription"
        size="small" @click="$emit('cancelSubscription')">
        {{ $t('cancelSubscription') }}
      </button>
      <button class="settings-button rounded shadow-md" v-if="subscription && isCanceled" id="resumeSubscription"
        size="small" @click="$emit('resumeSubscription')">
